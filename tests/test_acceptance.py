...
class BackportToolTestCase(unittest.TestCase):
    ...
    def test_backport_validation_files(self):
        ...
        for case in validation_cases:
            ...
            before_file = os.path.join(case_dir, 'before_file_hash', os.listdir(os.path.join(case_dir, 'before_file_hash'))[0])
            after_file = os.path.join(case_dir, 'after_file_hash', os.listdir(os.path.join(case_dir, 'after_file_hash'))[0])
            target_file = os.path.join(case_dir, 'target_file_hash', os.listdir(os.path.join(case_dir, 'target_file_hash'))[0])

            # Run the tool against the validation files
            try:
                subprocess.run(f'{self.tool_path} {before_file} {after_file} {target_file}', shell=True, check=True)
